<?php
/* $Id: zip.lib.php,v 1.1.1.1 2012/09/14 16:01:08 d302112 Exp $ */
...
class zipfile
{
     ...
     * @access public
     */
    function addFiles($base, $files /*Only Pass Array*/)
    {
        foreach($files as $file)
        {
		    if (is_file($base . '/' . $file)) //directory check
		    {
            ...
            {
                if ($handle = opendir($base . '/' . $file))
                {
                    while (FALSE !== ($f = readdir($handle)))
	                {
                        if ($f != "." && $f != "..")
		                {
                            $this->addFiles($base, array($file.'/'.$f));
                        }
                    }
                    closedir($handle);
                }
