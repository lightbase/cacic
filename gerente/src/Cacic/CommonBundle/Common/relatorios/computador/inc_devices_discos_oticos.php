...
if (!$_SESSION['devices_discos_oticos'])
	$_SESSION['devices_discos_oticos'] = false;
if ($exibir == 'devices_discos_oticos')
	{
	$_SESSION['devices_discos_oticos'] = !($_SESSION['devices_discos_oticos']);
	}
else
	{
	$_SESSION['devices_discos_oticos'] = false;
...
<table width="94%" border="0" align="center" cellpadding="0" cellspacing="1">
  ...
  <tr> 
    <td bgcolor="#E1E1E1" class="cabecalho_tabela">&nbsp;<a href="computador.php?exibir=devices_discos_oticos&te_node_address=<? echo $_GET['te_node_address']?>&id_so=<? echo $_GET['id_so']?>"> 
      <img src="../../imgs/<? if($_SESSION['devices_discos_oticos'] == true) echo 'menos';
  ...
  <?
		if ($_SESSION['devices_discos_oticos'] == true) {
		// EXIBIR INFORMAÇÕES DAS UNIDADES DE DISCO DO COMPUTADOR
			...
			if (mysql_result($result_acoes, 0, "cs_situacao") <> 'N') {
    			$query = "SELECT 	* 
						  FROM 		devices_discos_oticos
					  	  WHERE 	te_node_address = '". $_GET['te_node_address'] ."' AND 
					  				id_so = '". $_GET['id_so'] ."'";
			$result_disco = mysql_query($query) or die('Erro na consulta à tabela devices_discos_oticos ou sua sessão expirou!');
