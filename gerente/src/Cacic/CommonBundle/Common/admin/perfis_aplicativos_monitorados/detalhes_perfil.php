	...
	if ($_SESSION['cs_nivel_administracao']==1)
		...
		$query = "UPDATE 	perfis_aplicativos_monitorados 
				  SET 		nm_aplicativo 						= '".$v_nm_aplicativo."',  
				  			te_dir_padrao_w9x 					= '".mysql_real_escape_string($_POST['frm_te_dir_padrao_w9x'])."',  
							te_dir_padrao_wnt 					= '".mysql_real_escape_string($_POST['frm_te_dir_padrao_wnt'])."',  
			  				cs_car_inst_w9x 					= '".$_POST['frm_cs_car_inst_w9x']."',   
							cs_car_inst_wnt 					= '".$_POST['frm_cs_car_inst_wnt']."',  
			  				te_car_inst_w9x 					= '".mysql_real_escape_string($_POST['frm_te_car_inst_w9x'])."',  
							te_car_inst_wnt 					= '".mysql_real_escape_string($_POST['frm_te_car_inst_wnt'])."',  
				  			cs_car_ver_w9x 						= '".$_POST['frm_cs_car_ver_w9x']."',  
							cs_car_ver_wnt 						= '".$_POST['frm_cs_car_ver_wnt']."',  
				  			te_car_ver_w9x 						= '".mysql_real_escape_string($_POST['frm_te_car_ver_w9x'])."',  
							te_car_ver_wnt 						= '".mysql_real_escape_string($_POST['frm_te_car_ver_wnt'])."',  
			  				te_arq_ver_eng_w9x 					= '".mysql_real_escape_string($_POST['frm_te_arq_ver_eng_w9x'])."',  
							te_arq_ver_pat_w9x 					= '".mysql_real_escape_string($_POST['frm_te_arq_ver_pat_w9x'])."',  	  
				  			te_arq_ver_eng_wnt 					= '".mysql_real_escape_string($_POST['frm_te_arq_ver_eng_wnt'])."',  
							te_arq_ver_pat_wnt 					= '".mysql_real_escape_string($_POST['frm_te_arq_ver_pat_wnt'])."',  	  			  
				  			cs_ide_licenca 						= '".$_POST['frm_cs_ide_licenca']."',  
							te_ide_licenca 						= '".mysql_real_escape_string($_POST['frm_te_ide_licenca'])."',  	  
			  				id_so 								= '".$_POST['frm_id_so']."',  
							te_descritivo 						= '".mysql_real_escape_string($_POST['frm_te_descritivo'])."',  
			  				dt_atualizacao 						= now(),
...
<div align="center"><br>
		...
		while ($row_so=mysql_fetch_array($sql_result))
          ...
          <tr> 
            <td nowrap> 
            <select name="frm_cs_ide_licenca" id="select6" onChange="SetaDescGrupo(this.options[selectedIndex].id,'Ajuda1')" class="normal" onFocus="SetaClassDigitacao(this);" onBlur="SetaClassNormal(this);" <? echo ($_SESSION['cs_nivel_administracao']<>1?'disabled':'')?>>
            <option value="0" id=""></option>
            <option value="1" <? if ($row['cs_ide_licenca']=='1') echo 'selected';?>  id="<?=$oTranslator->_('Exemplo: HKEY_LOCAL_MACHINE\Software\Dataprev\Cacic2\id_versao');?>"><?=$oTranslator->_('Caminho\Chave\Valor em Registry');?></option>
            <option value="2" <? if ($row['cs_ide_licenca']=='2') echo 'selected';?>  id="<?=$oTranslator->_('Exemplo: Arquivos de Programas\Cacic\Dados\config.ini/Patrimonio/nu_CPU');?>"><?=$oTranslator->_('Caminho\nome do arquivo/Secao/Chave de Arquivo INI');?></option>
            </select> 
            <br> <input name="frm_te_ide_licenca" type="text" id="frm_te_ide_licenca" value="<? echo $row['te_ide_licenca'];?>" size="80" maxlength="150" class="normal" onFocus="SetaClassDigitacao(this);" onBlur="SetaClassNormal(this);" <? echo ($_SESSION['cs_nivel_administracao']<>1?'readonly':'')?>> 
          ...
          <tr> 
            <td nowrap>
            <select name="frm_cs_car_inst_w9x" id="select17" onChange="SetaDescGrupo(this.options[selectedIndex].id,'Ajuda2')" class="normal" onFocus="SetaClassDigitacao(this);" onBlur="SetaClassNormal(this);" <? echo ($_SESSION['cs_nivel_administracao']<>1?'disabled':'')?>>
            <option value="0" id=""></option>
            <option value="1" <? if ($row['cs_car_inst_w9x']=='1') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: Cacic\modulos\ger_cols.exe');?>"><?=$oTranslator->_('Nome de executavel');?></option>
            <option value="2" <? if ($row['cs_car_inst_w9x']=='2') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: Arquivos de Programas\Cacic\Dados\config.ini/Patrimonio/nu_CPU');?>"><?=$oTranslator->_('Caminho\nome do arquivo/Secao/Chave de Arquivo INI');?></option>
            <option value="3" <? if ($row['cs_car_inst_w9x']=='3') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: HKEY_LOCAL_MACHINE\Software\Dataprev\Cacic2\id_versao');?>"><?=$oTranslator->_('Caminho\Chave\Valor em Registry');?></option>
            </select> 
            <br> <input name="frm_te_car_inst_w9x" type="text" id="frm_te_car_inst_w9x3" size="80" maxlength="150" value="<? echo $row['te_car_inst_w9x'];?>" class="normal" onFocus="SetaClassDigitacao(this);" onBlur="SetaClassNormal(this);" <? echo ($_SESSION['cs_nivel_administracao']<>1?'readonly':'')?>> 
          ...
          <tr> 
            <td nowrap> 
            <select name="frm_cs_car_ver_w9x" id="select18" onChange="SetaDescGrupo(this.options[selectedIndex].id,'Ajuda3')" class="normal" onFocus="SetaClassDigitacao(this);" onBlur="SetaClassNormal(this);" <? echo ($_SESSION['cs_nivel_administracao']<>1?'disabled':'')?>>
            <option value="0" id=""></option>
            <option value="1" <? if ($row['cs_car_ver_w9x']=='1') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: Cacic\modulos\ger_cols.exe');?>"><?=$oTranslator->_('Data de arquivo');?></option>
            <option value="2" <? if ($row['cs_car_ver_w9x']=='2') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: HKEY_LOCAL_MACHINE\Software\Dataprev\Cacic2\id_versao');?>"><?=$oTranslator->_('Caminho\Chave\Valor em Registry');?></option>
            <option value="3" <? if ($row['cs_car_ver_w9x']=='3') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: Arquivos de Programas\Cacic\Dados\config.ini/Patrimonio/nu_CPU');?>"><?=$oTranslator->_('Caminho\nome do arquivo/Secao/Chave de Arquivo INI');?></option>
            <option value="4" <? if ($row['cs_car_ver_w9x']=='4') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: Cacic\cacic.exe');?>"><?=$oTranslator->_('Versao de executavel');?></option>
            </select> 
            <br>
            <input name="frm_te_car_ver_w9x" type="text" id="frm_te_car_ver_w9x3" size="80" maxlength="150" value="<? echo $row['te_car_ver_w9x'];?>" class="normal" onFocus="SetaClassDigitacao(this);" onBlur="SetaClassNormal(this);" <? echo ($_SESSION['cs_nivel_administracao']<>1?'readonly':'')?>> 
          ...
          <tr> 
            <td nowrap> 
            <select name="frm_cs_car_inst_wnt" id="select19" onChange="SetaDescGrupo(this.options[selectedIndex].id,'Ajuda4')" class="normal" onFocus="SetaClassDigitacao(this);" onBlur="SetaClassNormal(this);" <? echo ($_SESSION['cs_nivel_administracao']<>1?'disabled':'')?>>
                <option value="0" id=""></option>
                <option value="1" <? if ($row['cs_car_inst_wnt']=='1') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: Cacic\modulos\ger_cols.exe');?>"><?=$oTranslator->_('Nome de executavel');?></option>
                <option value="2" <? if ($row['cs_car_inst_wnt']=='2') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: Arquivos de Programas\Cacic\Dados\config.ini/Patrimonio/nu_CPU');?>"><?=$oTranslator->_('Caminho\nome do arquivo/Secao/Chave de Arquivo INI');?></option>
                <option value="3" <? if ($row['cs_car_inst_wnt']=='3') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: HKEY_LOCAL_MACHINE\Software\Dataprev\Cacic2\id_versao');?>"><?=$oTranslator->_('Caminho\Chave\Valor em Registry');?></option>
            </select>
            <br>
            <input name="frm_te_car_inst_wnt" type="text" id="frm_te_car_inst_wnt3" size="80" maxlength="150" value="<? echo $row['te_car_inst_wnt'];?>" class="normal" onFocus="SetaClassDigitacao(this);" onBlur="SetaClassNormal(this);" <? echo ($_SESSION['cs_nivel_administracao']<>1?'readonly':'')?>> 
          ...
          <tr> 
            <td nowrap> <select name="frm_cs_car_ver_wnt" id="select20" onChange="SetaDescGrupo(this.options[selectedIndex].id,'Ajuda5')" class="normal" onFocus="SetaClassDigitacao(this);" onBlur="SetaClassNormal(this);" <? echo ($_SESSION['cs_nivel_administracao']<>1?'disabled':'')?>>
                <option value="0" id=""></option>
                <option value="1" <? if ($row['cs_car_ver_wnt']=='1') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: Cacic\modulos\ger_cols.exe');?>"><?=$oTranslator->_('Data de arquivo');?></option>
                <option value="2" <? if ($row['cs_car_ver_wnt']=='2') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: HKEY_LOCAL_MACHINE\Software\Dataprev\Cacic2\id_versao');?>"><?=$oTranslator->_('Caminho\Chave\Valor em Registry');?></option>
                <option value="3" <? if ($row['cs_car_ver_wnt']=='3') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: Arquivos de Programas\Cacic\Dados\config.ini/Patrimonio/nu_CPU');?>"><?=$oTranslator->_('Caminho\nome do arquivo/Secao/Chave de Arquivo INI');?></option>
                <option value="4" <? if ($row['cs_car_ver_wnt']=='4') echo 'selected';?> id="<?=$oTranslator->_('Exemplo: Cacic\cacic.exe');?>"><?=$oTranslator->_('Versao de executavel');?></option>
              </select>
              <br> <input name="frm_te_car_ver_wnt" type="text" id="frm_te_car_ver_wnt3" size="80" maxlength="150" value="<? echo $row['te_car_ver_wnt'];?>" class="normal" onFocus="SetaClassDigitacao(this);" onBlur="SetaClassNormal(this);" <? echo ($_SESSION['cs_nivel_administracao']<>1?'readonly':'')?>> 
  ...
  <p align="center"> 
    <input name="GravaAlteracoes" type="submit" value="  <?=$oTranslator->_('Gravar alteracoes');?>  " onClick="return Confirma('<?=$oTranslator->_('Confirma Informacoes para perfil de sistema monitorado?');?>'),SelectAll(this.form.elements['list1[]']),SelectAll(this.form.elements['list2[]']) " <? echo ($_SESSION['cs_nivel_administracao']<>1 && $_SESSION['cs_nivel_administracao']<>3?'disabled':'')?>>
    &nbsp; &nbsp; 
