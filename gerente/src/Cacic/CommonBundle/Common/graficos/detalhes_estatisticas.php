...
<table width="100%" cellspacing="0" cellpadding="0" align="center">
  		<tr valign="top"> 
    	<td><div align="center"><strong><h3><?php echo $_GET['te_title'];?></h3></strong></div></td>
  		</tr>
  		<tr> 
    	<td>&nbsp;</td>
  		</tr>
	...
	while ($row = mysql_fetch_array($result_desdobra_estatisticas))
		...
		?>
  		<tr> 
    	<td bgcolor="#CCCCCC"><div align="center">Local: <strong><?php echo $row['sg_local'].' - '.$row['nm_local'];?></strong>
        </div></td>
  		</tr>	
  		<tr> 
    	<td>
		<?php echo '<a href="../relatorios/software/rel_software.php?orderby='.($_GET['in_grafico']=='acessos'?'6':'4').'&principal='.$_GET['in_grafico'].'&id_local='.$row['id_local'].'&sg_local='.$row['sg_local'].'&nm_local='.$row['nm_local'].'">';		
		$where = ' AND redes.id_local = '.$row['id_local']. ' ';		

		if (substr_count($_GET['te_exibe_graficos'],'['.$_GET['in_grafico'].']')>0)
			{
			?>
			<img src="pie_<?php echo $_GET['in_grafico'];?>.php?where=<?php echo $where;?>" border="no" title="CLIQUE PARA LISTAR ESSES COMPUTADORES">
			<?php
			}
		else
