...
//								ImageString($im, $FontNumber, 5, $MeuY, $key. ': ' . $value . ' ('.number_format(($value / $TotalArrayValues) * 100, 1).'%)', $label_color[$valuecounter % count($label_color)]);
								ImageString($im, $FontNumber, 5, $MeuY, $key . ': ' . str_pad($value,6,' ',STR_PAD_LEFT) . '('.str_pad(number_format(($value / $TotalArrayValues) * 100, 1),5,' ',STR_PAD_LEFT).'%)', $label_color[$valuecounter % count($label_color)]);
							if ($CreatePie)
								{							
						...
						else 
						{
							...
							if ($Legend) 
							{
								ImageString($im, $FontNumber, 5, round((ImageFontHeight($FontNumber) * .5) + ($valuecounter * 1.5 * ImageFontHeight($FontNumber))), 'Other ('.str_pad(number_format((($TotalArrayValues - $ValuesSoFar) / $TotalArrayValues) * 100, 1),4,' ',STR_PAD_LEFT).'%)', $line_color);
							}
							break;
					    }
						$valuecounter++;
...
//				If ($Legend) ImageString($im, $FontNumber, 6, round((ImageFontHeight($FontNumber) * .5) + ($valuecounter * 1.5 * ImageFontHeight($FontNumber))), str_pad('Total',strlen($key),'.',STR_PAD_RIGHT) . ': ' . str_pad($TotalArrayValues,6,' ',STR_PAD_LEFT) . ' ( 100%)', '9933FF');
				If ($Legend) 
					{
					$MeuYLinha = $MeuY + ImageFontHeight($FontNumber) + 2;
					$TamanhoLinha = 255;
					...
					ImageString($im, $FontNumber, 6, round((ImageFontHeight($FontNumber) * .5) + ($valuecounter * 1.5 * ImageFontHeight($FontNumber))), str_pad('Total',strlen($key),'.',STR_PAD_RIGHT) . ': ' . str_pad($TotalArrayValues,6,' ',STR_PAD_LEFT) . '(100.0%)', '999999');				
					}
