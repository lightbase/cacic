...
class cacic {
  ...
  class timezone inherits cacic::config {
    package { "tzdata":
      ensure => latest
    }
  ...
  class user inherits cacic::config {
    exec { 'add user':
  ...
  class apache_install inherits cacic::config {
  ...
  class postgresql_install inherits cacic::config {
...
define cacic::database (
  ...
  postgresql::server::db { "${db_name}":
    ...
    require  => [Class['postgresql::server'], Postgresql::Server::Role["${db_user}"]]
  }
}

class cacic::website (

) inherits cacic::config {
  require cacic::software
  require cacic::timezone
  require cacic::user
  require cacic::apache_install
  require cacic::postgresql_install
  require cacic::php_install

  # Database
  cacic::database { "${db_name}":
    db_name => $db_name,
  ...
  exec { 'cache clear':
    ...
    cwd => "/home/${user}/projects/${domain_name}",
    user => $user,
    require => Exec["composer-install-${domain_name}"]
  }
