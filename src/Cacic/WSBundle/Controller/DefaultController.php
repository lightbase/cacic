...
class DefaultController extends Controller
{
    ...
    public function testAction( Request $request )
    {
        ...
        if (empty($netmask) || (empty($te_node_address)) || empty($so)) {
            $this->get('logger')->error("Erro na operação de getTest. Máscara = $netmask. MAC = $te_node_address. SO = $te_so");
    ...
    public function configAction( Request $request )
    {
         ...
         */
        if (empty($netmask) || (empty($te_node_address)) || empty($so)) {
            $this->get('logger')->error("Erro na operação de getConfig. Máscara = $netmask. MAC = $te_node_address. SO = $request->get( 'te_so' )");
