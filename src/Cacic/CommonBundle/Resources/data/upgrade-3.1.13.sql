...
BEGIN
  ...
  FOR classe IN select *
    ...
    FOR prop IN select distinct nm_property_name
      ...
      sql := 'ALTER TABLE relatorio_coleta ADD COLUMN '||lower(classe.nm_class_name)||'_'||lower(prop.nm_property_name)||' VARCHAR;
        ALTER TABLE relatorio_coleta ADD COLUMN '||lower(classe.nm_class_name)||'_'||lower(prop.nm_property_name)||'_data VARCHAR';
  ...
  FOR comp IN select distinct c.id_computador
              from computador c
              where c.ativo = 't' or c.ativo IS NULL LOOP
  ...
  FOR comp IN select c.id_computador_coleta,
                c.dt_hr_inclusao,
                c.id_computador,
                p.nm_property_name,
                cl.nm_class_name,
                c.te_class_property_value
