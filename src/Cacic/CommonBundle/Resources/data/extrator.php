...
function extrair($table, $query){
    ...
    // $query = "SELECT colunas_velhas AS colunas_novas FROM tabela_velha"
...
function create_temptables(){
    ...
    mysql_query("DROP TABLE IF EXISTS new_computador");

    mysql_query("CREATE TABLE new_redes (id_rede int(11) NOT NULL, id_ip_rede char(15) NOT NULL, id_local int(11), PRIMARY KEY (id_rede))");
    mysql_query("CREATE TABLE new_computador (id_computador int(11) NOT NULL, id_so int(11) NOT NULL, te_node_address char(17) NOT NULL, te_ip char(15) DEFAULT NULL, PRIMARY KEY (id_computador))");

    extrair("new_redes", "SELECT DISTINCT @s:=@s+1 id_rede, id_ip_rede, id_local FROM redes, (SELECT @s:=0) AS s");
    extrair("new_computador", "SELECT DISTINCT @s:=@s+1 id_computador, id_so, te_node_address, te_ip FROM computadores, (SELECT @s:=0) AS s");

    mysql_query("LOAD DATA LOCAL INFILE 'new_redes.csv' INTO TABLE new_redes FIELDS TERMINATED BY ';' ENCLOSED BY '\"' LINES TERMINATED BY '\n' IGNORE 1 LINES");
    mysql_query("LOAD DATA LOCAL INFILE 'new_computador.csv' INTO TABLE new_computador FIELDS TERMINATED BY ';' ENCLOSED BY '\"' LINES TERMINATED BY '\n' IGNORE 1 LINES");

}

$tabelas = array(
    // "nome_da_tabela_no_cacic3" => "SELECT colunas_velhas AS colunas_novas FROM tabela_velha"
    ...
    "acao_excecao" => "SELECT acao_excecao.te_node_address, acao_excecao.id_acao, new_redes.id_rede FROM acoes_excecoes AS acao_excecao INNER JOIN new_redes ON (acao_excecao.id_local=new_redes.id_local)",
    "acao_rede" => "SELECT acao_rede.id_acao, new_redes.id_rede, acao_rede.dt_hr_coleta_forcada FROM acoes_redes AS acao_rede INNER JOIN new_redes ON (acao_rede.id_local=new_redes.id_local AND acao_rede.id_ip_rede=new_redes.id_ip_rede)",
    "acao_so" => "SELECT acao_so.id_acao, new_redes.id_rede, acao_so.id_so FROM acoes_so AS acao_so INNER JOIN new_redes ON (acao_so.id_local=new_redes.id_local)",
    ...
    "aplicativo_rede" => "SELECT new_redes.id_rede, aplicativo_rede.id_aplicativo FROM aplicativos_redes AS aplicativo_rede INNER JOIN new_redes ON (aplicativo_rede.id_ip_rede=new_redes.id_ip_rede)",
    ...
    "computador" => "SELECT new_computador.id_computador, @s1:=NULL id_usuario_exclusao, computador.id_so, new_redes.id_rede, computador.te_nome_computador AS nm_computador, computador.te_node_address, computador.te_ip AS te_ip_computador, computador.dt_hr_inclusao, @s2:=NULL dt_hr_exclusao, computador.dt_hr_ult_acesso, computador.te_versao_cacic, computador.te_versao_gercols, computador.te_palavra_chave, computador.dt_hr_coleta_forcada_estacao, computador.te_nomes_curtos_modulos, computador.id_conta, @s3:=NULL te_debugging, @s4:=NULL te_ultimo_login, @s5:=NULL dt_debug FROM computadores AS computador INNER JOIN new_redes ON (computador.id_ip_rede=new_redes.id_ip_rede) INNER JOIN new_computador ON (computador.id_so=new_computador.id_so AND computador.te_node_address=new_computador.te_node_address), (SELECT @s:=0) AS s",
    ...
    "rede" => "SELECT new_redes.id_rede, rede.id_local, rede.id_servidor_autenticacao, rede.id_ip_rede AS te_ip_rede, rede.nm_rede, rede.te_observacao, rede.nm_pessoa_contato1, rede.nm_pessoa_contato2, rede.nu_telefone1, rede.te_email_contato2, rede.nu_telefone2, rede.te_email_contato1, rede.te_serv_cacic, rede.te_serv_updates, rede.te_path_serv_updates, rede.nm_usuario_login_serv_updates, rede.te_senha_login_serv_updates, rede.nu_porta_serv_updates, rede.te_mascara_rede, rede.dt_verifica_updates, rede.nm_usuario_login_serv_updates_gerente, rede.te_senha_login_serv_updates_gerente, rede.nu_limite_ftp, rede.cs_permitir_desativar_srcacic, @s1:=NULL te_debugging, @s2:=NULL dt_debug FROM redes AS rede INNER JOIN new_redes ON (rede.id_local=new_redes.id_local AND rede.id_ip_rede=new_redes.id_ip_rede)",
    "rede_grupo_ftp" => "SELECT rede_grupo_ftp.id_ftp, new_redes.id_rede, new_computador.id_computador, rede_grupo_ftp.nu_hora_inicio, rede_grupo_ftp.nu_hora_fim FROM redes_grupos_ftp AS rede_grupo_ftp INNER JOIN new_redes ON (rede_grupo_ftp.id_local=new_redes.id_local) AND rede_grupo_ftp.id_ip_rede=new_redes.id_ip_rede INNER JOIN new_computador ON (rede_grupo_ftp.id_ip_estacao=new_computador.te_ip)",
    "rede_versao_modulo" => "SELECT @s:=@s+1 id_rede_versao_modulo, new_redes.id_rede, rede_versao_modulo.nm_modulo, rede_versao_modulo.te_versao_modulo, rede_versao_modulo.dt_atualizacao, rede_versao_modulo.cs_tipo_so, rede_versao_modulo.te_hash FROM redes_versoes_modulos AS rede_versao_modulo INNER JOIN new_redes ON (rede_versao_modulo.id_local=new_redes.id_local) AND rede_versao_modulo.id_ip_rede=new_redes.id_ip_rede, (SELECT @s:=0) AS s",
    ...
    "srcacic_sessao" => "SELECT id_sessao AS id_srcacic_sessao, new_computador.id_computador, dt_hr_inicio_sessao, nm_acesso_usuario_srv, nm_completo_usuario_srv, te_email_usuario_srv, dt_hr_ultimo_contato FROM srcacic_sessoes INNER JOIN new_computador ON (new_computador.id_so=srcacic_sessoes.id_so_srv AND new_computador.te_node_address=srcacic_sessoes.te_node_address_srv)",
    ...
    "usb_log" => "SELECT @s:=@s+1 id_usb_log, id_vendor AS id_usb_vendor, id_device AS id_usb_device, new_computador.id_computador, dt_event, cs_event FROM usb_logs INNER JOIN new_computador ON (new_computador.id_so=usb_logs.id_so AND new_computador.te_node_address=usb_logs.te_node_address), (SELECT @s:=0) AS s",
