...
function createnewredes(){
    //Cria e popula a tabela temporaria "newredes"
...
$tabelas = array(
    ...
    "computador" => "SELECT @s:=@s+1 id_computador, @s1:=NULL id_usuario_exclusao, computador.id_so, newredes.id_rede, computador.te_nome_computador AS nm_computador, computador.te_node_address, computador.te_ip AS te_ip_computador, computador.dt_hr_inclusao, @s2:=NULL dt_hr_exclusao, computador.dt_hr_ult_acesso, computador.te_versao_cacic, computador.te_versao_gercols, computador.te_palavra_chave, computador.dt_hr_coleta_forcada_estacao, computador.te_nomes_curtos_modulos, computador.id_conta, @s3:=NULL te_debugging, @s4:=NULL te_ultimo_login, @s5:=NULL dt_debug FROM computadores AS computador INNER JOIN newredes ON computador.id_ip_rede=newredes.id_ip_rede, (SELECT @s:= 0) AS s",
    ...
    // "rede_grupo_ftp" => "SELECT rede_grupo_ftp.id_ftp, newredes.id_rede, id_computador, rede_grupo_ftp.nu_hora_inicio, rede_grupo_ftp.nu_hora_fim FROM redes_grupos_ftp AS rede_grupo_ftp INNER JOIN newredes ON rede_grupo_ftp.id_local=newredes.id_local AND rede_grupo_ftp.id_ip_rede=newredes.id_ip_rede",
