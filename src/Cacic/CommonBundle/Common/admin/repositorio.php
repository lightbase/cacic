...
if ($_REQUEST['nm_arquivo'])
	{if(!@unlink(CACIC_PATH . CACIC_PATH_RELATIVO_DOWNLOADS . $_REQUEST['nm_arquivo']))
		{
		echo '<script>alert("'.$oTranslator->_('Nao foi possivel excluir o arquivo %1. Verifique as permissoes de escrita no diretorio ' . CACIC_PATH . CACIC_PATH_RELATIVO_DOWNLOADS . '!', array($_REQUEST['nm_arquivo'])).'")</script>';
		}
	else
		{
		conecta_bd_cacic();
    ...
    <td class="cabecalho"><?php echo $oTranslator->_('Repositorio');?></td>
	  </tr>
	  <tr> 
		
    <td class="descricao"><?php echo $oTranslator->_('kciq_msg Repositorio help');?></td>
	  </tr>
	</table>
  <div align="center"> 
    <table width="85%" border="0" align="center" cellpadding="0" cellspacing="1">
      ...
      <tr> 
        <td nowrap align="center" colspan="4" class="<?php echo $v_classe; ?>"><br>
          <?php echo $oTranslator->_('Conteudo do repositorio');?>:</td>
      </tr>
      <tr> 
      ...
      <tr> 
        <td nowrap colspan="2"><table border="1" align="center" cellpadding="2" bordercolor="#999999">
            <tr bgcolor="#FFFFCC"> 
              ...
              <td align="right" colspan="3" nowrap bgcolor="#EBEBEB" class="cabecalho_tabela"><?php echo $oTranslator->_('Data e hora');?></td>
            </tr>
            <?php if ($handle = opendir(CACIC_PATH . CACIC_PATH_RELATIVO_DOWNLOADS)) 
		{
		...
		for ($cnt_arquivos = 0; $cnt_arquivos < count($v_nomes_arquivos); $cnt_arquivos++)
			{
			$v_dados_arquivo = lstat(CACIC_PATH . CACIC_PATH_RELATIVO_DOWNLOADS . $v_nomes_arquivos[$cnt_arquivos]);
			echo '<tr>';
			?>
			<td><a href="repositorio.php?nm_arquivo=<?php echo $v_nomes_arquivos[$cnt_arquivos];?>" onClick="return Confirma('<?php echo $oTranslator->_('Confirma exclusao?');?><?php echo $v_nomes_arquivos[$cnt_arquivos];?>)');"><img src="../imgs/lixeira.ico" width="20" height="20" border="0"></a></td>
			<?php echo '<td>'.$v_nomes_arquivos[$cnt_arquivos].'</td>';										
