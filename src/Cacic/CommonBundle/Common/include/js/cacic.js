
var ie4 = (document.all) ? true : false;
var ns4 = (document.layers) ? true : false;
var ns6 = (document.getElementById && !document.all) ? true : false;

function SetaClassDigitacao(obj)
	{
	obj.style.backgroundColor='#CCCCCC';
	}

function SetaClassNormal(obj)
	{
	obj.style.backgroundColor='#FFFFFF';
	}
function PreencheFormChecksRedes()
	{
	var Formulario = window.document.forms[0];
	var arrRede;
	var strSelectedValues;
	var strNotSelectedValues;
	strSelectedValues 	 = "";
	strNotSelectedValues = "";

	for (i = 0; i < Formulario.length; i++) 
		if (Formulario[i].type == 'checkbox' && (Formulario[i].name).substring(0,5) == 'rede_')
			{
			arrRede = (Formulario[i].name).split('_');
			if (Formulario[i].checked)
				{			
				strSelectedValues = strSelectedValues + (strSelectedValues != "" ? "," : "");			
				strSelectedValues = strSelectedValues + arrRede[1];								
				}
			else
				{
				strNotSelectedValues = strNotSelectedValues + (strNotSelectedValues != "" ? "," : "");			
				strNotSelectedValues = strNotSelectedValues + arrRede[1];								
				}
			}
	document.forma.elements['frmRedes_NaoSelecionadas'].value = strNotSelectedValues;
	document.forma.elements['frmRedes_Selecionadas'].value	  = strSelectedValues;
	}		

function preparaEnvio()
	{// Percorrerei os forms e encontrarei os campos NOME e SENHA para criptografia... (Anderson Peterle - 04/09/2006)
	for (i=0;i<window.document.forms.length;i++)
		for (j=0;j<window.document.forms[i].elements.length;j++)
			if ((window.document.forms[i].elements[j].name == 'frm_nm_usuario_acesso' ||
				 window.document.forms[i].elements[j].name == 'frm_te_senha') &&
				(window.document.forms[i].elements[j].type  == 'text' ||
				 window.document.forms[i].elements[j].type  == 'password') && 
				 window.document.forms[i].elements[j].value != '')
				// Codificação dos campos NOME e SENHA para submissão pelo método POST
				window.document.forms[i].elements[j].value = encode64(window.document.forms[i].elements[j].value);		
	}

function SetaCampo(p_campo)
	{
	for (i=0;i<window.document.forms.length;i++)
		for (j=0;j<window.document.forms[i].elements.length;j++)
			if (window.document.forms[i].elements[j].name == p_campo) window.document.forms[i].elements[j].focus();

	return true;
	}		


function Ajuda(p_campo_destino,p_frase) 
	{
	var formulario = window.document.forms[0];
	formulario[p_campo_destino].value = p_frase;
	return true;		
	}

function Confirma(p_pergunta) 
	{
	if (confirm(p_pergunta))
		return true;

	return false;
	}	

function MostraLayer(strLayerName) 
	{
	if      (ie4) 
		document.all[strLayerName].style.visibility = "visible";
	else if (ns4) 
		document.layers[strLayerName].visibility = "show";
	else if (ns6) 
		document.getElementById([strLayerName]).style.display = "block";
	}
	
function EscondeLayer(strLayerName)
	{
	if      (ie4) 
		document.all[strLayerName].style.visibility = "hidden";
	else if (ns4) 
		document.layers[strLayerName].visibility = "hide";
	else if (ns6) 
		document.getElementById([strLayerName]).style.display = "none";
	}
function returnObjById(pStrId)
    {
    var returnObj = new Object;    
    
    if (document.getElementById)
        returnObj = document.getElementById(pStrId);
    else if (document.all)
        returnObj = document.all[pStrId];
    else if (document.layers)
        returnObj = document.layers[pStrId];
    
    return returnObj;
    }
function returnObjByName(pStrName)
    {
    var returnObj = new Object;    
    
    if (document.getElementByName)
        returnObj = document.getElementByName(pStrName);
    else if (document.all)
        returnObj = document.all[pStrName];
    else if (document.layers)
        returnObj = document.layers[pStrName];
    
    return returnObj;
    }

function EscreveNaLayer(strLayerName,strText) 
	{
	if      (ie4) 
		document.all[strLayerName].innerHTML = strText;
	else if (ns4) 
		{
		document[strLayerName].document.write(strText);
		document[strLayerName].document.close();
		}
	else if (ns6) 
		{
		over = document.getElementById([strLayerName]);
		range = document.createRange();
		range.setStartBefore(over);
		domfrag = range.createContextualFragment(strText);
		while (over.hasChildNodes()) 
			over.removeChild(over.lastChild);

		over.appendChild(domfrag);
	   }
	}

function Trim(str)
	{
	return str.replace(/^\s+|\s+$/g,"");
	}

function VerRedeMascara(strFormName,boolPreencheIPs,boolConfirma) 
	{
	var frmForm 		 = document.getElementById(strFormName);
	var arrTeIpRede      = (frmForm.frm_te_ip_rede.value).split('.');
	var arrTeMascaraRede = (frmForm.frm_te_mascara_rede.value).split('.');
	
  	var intTesteTeIpRede = TestaIP(arrTeIpRede[0],arrTeIpRede[1],arrTeIpRede[2],arrTeIpRede[3]);  	
	if (intTesteIdIpRede > 0) 
		{
		alert(CACIC_JS_MSG_IP_REDE_INVALIDA); 
		frmForm.frm_te_ip_rede.select();			
		frmForm.frm_te_ip_rede.focus();		
		return false; 
		}
		
  	var intTesteTeMascaraRede = TestaMascara(arrTeMascaraRede[0],arrTeMascaraRede[1],arrTeMascaraRede[2],arrTeMascaraRede[3]);  		
  	if (intTesteTeMascaraRede > 0) 
		{ 
		alert(CACIC_JS_MSG_MASCARA_REDE_INVALIDA); 
		frmForm.frm_te_mascara_rede.select();			
		frmForm.frm_te_mascara_rede.focus();		
		return false; 
		}

	
  	// Cálculo do endereço de rede
  	var strOctetoRede1		=	arrTeIpRede[0] & arrTeMascaraRede[0];
  	var strOctetoRede2		=	arrTeIpRede[1] & arrTeMascaraRede[1];
  	var strOctetoRede3		=	arrTeIpRede[2] & arrTeMascaraRede[2];
  	var strOctetoRede4		=  (arrTeIpRede[3] & arrTeMascaraRede[3])+1;
	
  	// Cálculo do endereço de broadcast
  	var strOctetoMascara1	=	arrTeIpRede[0] | (arrTeMascaraRede[0] ^ 255);
  	var strOctetoMascara2	=	arrTeIpRede[1] | (arrTeMascaraRede[1] ^ 255);
  	var strOctetoMascara3	=	arrTeIpRede[2] | (arrTeMascaraRede[2] ^ 255);
  	var strOctetoMascara4	=  (arrTeIpRede[3] | (arrTeMascaraRede[3] ^ 255))-1;
	
	var strIPInicio      = strOctetoRede1    + '.' + strOctetoRede2    + '.' + strOctetoRede3    + '.' + strOctetoRede4;
	var strIPFim         = strOctetoMascara1 + '.' + strOctetoMascara2 + '.' + strOctetoMascara3 + '.' + strOctetoMascara4;

	if (boolPreencheIPs)
		PreencheIPs(strFormName,strIPInicio,strIPFim);

	if (boolConfirma && Confirma(CACIC_JS_MSG_ATENCAO+'\n'+CACIC_JS_MSG_REDE_AVISO+' '+ strIPInicio+' '+CACIC_JS_MSG_A+' '+strIPFim+'\n\n'+CACIC_JS_MSG_CONFIRMA))
		{
		PreencheIPs(strFormName,strIPInicio,strIPFim);			
		return true;
		}
	else
		{
		if (!boolPreencheIPs)
			{
			frmForm.frm_te_mascara_rede.select();			
			frmForm.frm_te_mascara_rede.focus();
			}
		return false;
		}
	}

function PreencheIPs(strFormName,strIPInicio,strIPFim)
	{
	var frmForm = document.getElementById(strFormName);		
	frmForm.frm_id_ip_inicio.value 	= strIPInicio;
	frmForm.frm_id_ip_fim.value 	= strIPFim;
	}
	
function TestaIP(IP1, IP2, IP3, IP4) 
	{
  	if ((IP1 > 255) || (IP1 < 1) || (IP1=="") || (IP1==undefined))
		return 1;

	if ((IP2 > 255) || (IP2 < 0) || (IP2=="") || (IP2==undefined))
		return 2;

	if ((IP3 > 255) || (IP3 < 0) || (IP3=="") || (IP3==undefined))
		return 3;

	if ((IP4 > 255) || (IP4 < 0) || (IP4=="") || (IP4==undefined))
		return 4;
		
  	return 0;
	}
	
function TestaMascara(IP1, IP2, IP3, IP4) 
	{
  	if ((IP1 > 255) || (IP1 < 0) || (IP1=="") || (IP1==undefined)) 
		return 1;
		
  	if ((IP2 > 255) || (IP2 < 0) || (IP2=="") || (IP2==undefined))
		return 2;
		
	if ((IP3 > 255) || (IP3 < 0) || (IP3=="") || (IP3==undefined)) 
		return 3;
		
  	if ((IP4 > 255) || (IP4 < 0) || (IP4=="") || (IP4==undefined))
		return 4;
		
  	var IPX =5;

	// Determine where IP changes
  	if (IP1 < 255) 
		{
    	if((IP2 > 0) || (IP3 > 0) || (IP4 > 0))
			return 5;
	    IPX = IP1;
    	} 
	else 
		{
      	if (IP2 < 255) 
			{
        	if((IP3 > 0) || (IP4 > 0))
				return 6;
        	IPX = IP2;
      		} 
		else 
			{
        	if (IP3 < 255) 
				{
          		if ((IP4 > 0))
					return 7;
	  			IPX = IP3;
        		} 
			else
				IPX = IP4;
      		}
    	}

	// Valida o IPX
    switch (IPX) 
		{
      	case "255":
      	case "128":
      	case "192":
      	case "224":
      	case "240":
      	case "248":
      	case "252":
      	case "254":
      	case "0":
        return 0;
      default:
        return 8;
    	} 
	return 0;
  }	

