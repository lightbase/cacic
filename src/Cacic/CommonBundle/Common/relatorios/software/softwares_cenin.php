...
<body bgcolor="#FFFFFF" background="/cacic3/imgs/linha_v.gif">
<script language="JavaScript" type="text/javascript" src="http://<?php echo $_SERVER['HTTP_HOST'].'/cacic3/include/js/cacic.js';?>"></script>

<?php if (($_SESSION['ftr_consulta_cenin'] == 'todos') or ($_SESSION['ftr_consulta_cenin'] == '')) {
...
<td>
    ...
    <tr><td class="label">Siglas: CEACE, CENIN, CESAN, COAUS, CODIS, COINF, CORED, COREL, COSERV, COSEV, SATUS, SAUTE, SEADI, SEANS, SECONS, SEGAB, SEGERE, SEIAD, SEMAD, SEMIR, SEOPR, SEPAS, SERAN, SESAP, SESAT, SESEG, SESEN, SESUP, SETES, SINCO</td>
    </tr>
  </table>
  </form>
<?php
$queryCENIN = " ((c.te_nome_computador LIKE '%CEACE%') OR 
	       (c.te_nome_computador LIKE '%CENIN%') OR 
	       (c.te_nome_computador LIKE '%CESAN%') OR 
	       (c.te_nome_computador LIKE '%COAUS%') OR 
	       (c.te_nome_computador LIKE '%CODIS%') OR 
	       (c.te_nome_computador LIKE '%COINF%') OR 
	       (c.te_nome_computador LIKE '%CORED%') OR 
	       (c.te_nome_computador LIKE '%COREL%') OR 
	       (c.te_nome_computador LIKE '%COSERV%') OR 
	       (c.te_nome_computador LIKE '%COSEV%') OR 
	       (c.te_nome_computador LIKE '%SATUS%') OR 
	       (c.te_nome_computador LIKE '%SAUTE%') OR 
	       (c.te_nome_computador LIKE '%SEADI%') OR 
	       (c.te_nome_computador LIKE '%SEANS%') OR 
	       (c.te_nome_computador LIKE '%SECONS%') OR 
	       (c.te_nome_computador LIKE '%SEGAB%') OR 
	       ...
	       (c.te_nome_computador LIKE '%SESAT%') OR 
	       (c.te_nome_computador LIKE '%SESEG%') OR 
	       (c.te_nome_computador LIKE '%SESEN%') OR 
	       (c.te_nome_computador LIKE '%SESUP%') OR 
	       (c.te_nome_computador LIKE '%SETES%') OR 
	       (c.te_nome_computador LIKE '%SINCO%')) ";

if ($_POST['consultar']) {
	if ($_SESSION['ftr_consulta_cenin'] == 'todos') {
		$query = "SELECT ss.id_software_inventariado as id_soft, 
		ss.nm_software_inventariado as nm_soft, count(*) as qtde,c.id_computador 
	...
	while($row = mysql_fetch_array($result)) {
	 ...
	 ?>
        <tr <?php if ($Cor) { echo 'bgcolor="#E1E1E1"'; } ?>> 
          ...
          <td nowrap>&nbsp;</td>
          <td nowrap class="opcao_tabela"><a href="rel_softwares_cenin.php?id_software_inventariado=<?php echo $row['id_soft'];?>&nm_software_inventariado=<?php echo $row['nm_soft'];?>" target="_blank"><?php echo $row['nm_soft']; ?></a></td>
          <td nowrap>&nbsp;</td>
