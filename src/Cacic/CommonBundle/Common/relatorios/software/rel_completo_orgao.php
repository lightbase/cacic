...
<?php
	 ...
	 while ($campos_resultOrgao=mysql_fetch_array($resultOrgao)) {
		if ($restricao) {
			$restricao = $restricao . " OR (a.te_nome_computador LIKE '" . $campos_resultOrgao['nomeComputador'] . "')";
		} else {
			$restricao = " AND ((a.te_nome_computador LIKE '" . $campos_resultOrgao['nomeComputador'] . "')";
		}
	 }
	 $query = "SELECT a.te_nome_computador, a.te_node_address, a.id_so,
		...
		WHERE a.te_node_address = c.id_computador  
