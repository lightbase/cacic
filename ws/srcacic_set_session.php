...
//LimpaTESTES();
GravaTESTES('SetSession: Valores Recebidos:');
...
if ($te_palavra_chave == $strTePalavraChave)
	...
	if (!$_POST['id_sessao'])
		...
		$arr_id_usuario_visitante   = explode('<REG>',DeCrypt($key,$iv,$_POST['id_usuario_visitante'],$v_cs_cipher,$v_cs_compress,$strPaddingKey));
		$arr_te_so_visitante 	   	= explode('<REG>',DeCrypt($key,$iv,$_POST['te_so_visitante']	 ,$v_cs_cipher,$v_cs_compress,$strPaddingKey));

		GravaTESTES('POST_id_usuario_visitante: '.DeCrypt($key,$iv,$_POST['id_usuario_visitante'],$v_cs_cipher,$v_cs_compress,$strPaddingKey));
		GravaTESTES('Tamanho de arr_id_usuario_visitante: '.count($arr_id_usuario_visitante));
		
		for ($i=0; $i < count($arr_id_usuario_visitante); $i++)
			{
			$te_so_visitante 		= $arr_te_so_visitante[$i]; 								
			$arr_id_so_visitante	= getValores('so','id_so','trim(te_so)="'.trim($te_so_visitante).'"');
			...
			*/
			...
			$query_SESSAO = "UPDATE srcacic_sessoes_logs 
							 ...
							 WHERE  id_sessao = ".$id_sessao." and
									id_usuario_visitante = ".$arr_id_usuario_visitante[$i]." AND
									id_so_visitante = ".$arr_id_so_visitante['id_so'] ;
/*." and
...
//GravaTESTES($retorno_xml_values);
